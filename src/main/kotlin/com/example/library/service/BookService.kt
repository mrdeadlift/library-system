...
class BookService(
     ...
     * 書籍一覧を取得（ページネーション対応）
     */
    fun findAll(pageNumber: Int = 0, pageSize: Int = 20): PagedResponse<BookResponse> {
     ...
     */
    fun searchByTitle(title: String, pageNumber: Int = 0, pageSize: Int = 20): PagedResponse<BookResponse> {
     ...
     */
    fun findByPublicationStatus(status: PublicationStatus, pageNumber: Int = 0, pageSize: Int = 20): PagedResponse<BookResponse> {
        require(pageNumber >= 0) { "ページ番号は0以上である必要があります" }
        require(pageSize > 0) { "ページサイズは1以上である必要があります" }
     ...
     */
    fun findByAuthorId(authorId: Long, pageNumber: Int = 0, pageSize: Int = 20): PagedResponse<BookResponse> {
        require(pageNumber >= 0) { "ページ番号は0以上である必要があります" }
        require(pageSize > 0) { "ページサイズは1以上である必要があります" }
     ...
     */
    @Transactional
    fun updatePublicationStatus(id: Long, newStatus: PublicationStatus): BookResponse {
        // 書籍の存在確認
        val existingBook = bookRepository.findById(id)
            ?: throw ResourceNotFoundException("指定されたID=$id の書籍が見つかりません")
     ...
     */
    @Transactional
    fun addAuthor(bookId: Long, authorId: Long): BookResponse {
        // 書籍の存在確認
     ...
     */
    @Transactional
    fun removeAuthor(bookId: Long, authorId: Long): BookResponse {
        // 書籍の存在確認
        val existingBook = bookRepository.findById(bookId)
            ?: throw ResourceNotFoundException("指定されたID=$bookId の書籍が見つかりません")
     ...
     */
    fun findPublishedBooks(pageNumber: Int = 0, pageSize: Int = 20): PagedResponse<BookResponse> {
     ...
     */
    fun findUnpublishedBooks(pageNumber: Int = 0, pageSize: Int = 20): PagedResponse<BookResponse> {
