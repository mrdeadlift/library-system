...
class AuthorServiceTest {
    ...
    fun `deleteById - 正常系`() {
        ...
        whenever(authorRepository.deleteById(1L)).thenReturn(true)
    ...
    @Test
    fun `deleteById - 削除に失敗`() {
        // Given
        whenever(authorRepository.existsById(1L)).thenReturn(true)
        whenever(authorRepository.deleteById(1L)).thenReturn(false)
        ...
        assertEquals("著者の削除に失敗しました。ID=1", exception.message)
    }
    ...
    fun `existsById - 正常系`() {
        ...
        // When & Then
        assertTrue(authorService.existsById(1L))
        assertFalse(authorService.existsById(999L))
    }
}
