...
class BookManagementIntegrationTest {
    ...
    fun `書籍管理システム全体の統合テスト - 実際の運用シナリオ`() {
        ...
        val author1Result = mockMvc.perform(post("/api/authors")
        ...
        val author2Result = mockMvc.perform(post("/api/authors")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        val book1Result = mockMvc.perform(post("/api/books")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        val book2Result = mockMvc.perform(post("/api/books")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        // === STEP 4: 別の著者の書籍を登録 ===
        val book3Request = BookCreateRequest(
            title = "容疑者Xの献身",
        ...
        val book3Result = mockMvc.perform(post("/api/books")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        // === STEP 9: 書籍の出版状況を変更 ===
        // 未出版書籍を出版済みに変更
        mockMvc.perform(patch("/api/books/$book1Id/publication-status")
        ...
        // === STEP 10: 共著書籍の作成 ===
        val collaborationBookRequest = BookCreateRequest(
            title = "共著作品",
        ...
        val collaborationResult = mockMvc.perform(post("/api/books")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        val author3Result = mockMvc.perform(post("/api/authors")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(put("/api/books/$book1Id")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        repeat(18) { i ->
            val additionalBookRequest = BookCreateRequest(
                title = "追加書籍${i + 1}",
        ...
        mockMvc.perform(patch("/api/books/$book2Id/publication-status")
            .param("status", "UNPUBLISHED"))
            .andExpect(status().isBadRequest)
        
        // 最後の著者を削除しようとする（エラーになることを確認）
        val singleAuthorBookRequest = BookCreateRequest(
            title = "単著書籍",
        ...
        val singleAuthorResult = mockMvc.perform(post("/api/books")
            .contentType(MediaType.APPLICATION_JSON)
    ...
    fun `データ整合性テスト - 著者削除時の書籍への影響`() {
        ...
        val authorResult = mockMvc.perform(post("/api/authors")
            .contentType(MediaType.APPLICATION_JSON)
        ...
        val bookResult = mockMvc.perform(post("/api/books")
            .contentType(MediaType.APPLICATION_JSON)
