...
class AuthorControllerTest {
    ...
    /**
     * DELETE /api/authors/{id} - 著者削除のテスト
     */
    @Test
    fun `deleteAuthor should return 204 when author is deleted successfully`() {
        // Given
        doNothing().whenever(authorService).deleteById(1L)

        // When & Then
        mockMvc
            .perform(delete("/api/authors/1"))
    ...
    /**
     * GET /api/authors/{id}/exists - 著者存在確認のテスト
     */
    @Test
    fun `checkAuthorExists should return true when author exists`() {
        // Given
        whenever(authorService.existsById(1L)).thenReturn(true)

        // When & Then
        mockMvc
            .perform(get("/api/authors/1/exists"))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.exists").value(true))

        verify(authorService).existsById(1L)
    }

    @Test
    fun `checkAuthorExists should return false when author does not exist`() {
        // Given
        whenever(authorService.existsById(999L)).thenReturn(false)

        // When & Then
        mockMvc
            .perform(get("/api/authors/999/exists"))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.exists").value(false))

        verify(authorService).existsById(999L)
    }
}
