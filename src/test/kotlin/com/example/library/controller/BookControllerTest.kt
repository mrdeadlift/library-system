...
class BookControllerTest {
     ...
     */
    @Test
    fun `deleteBook should return 204 when book is deleted successfully`() {
        // Given
        doNothing().whenever(bookService).deleteById(1L)
    ...
    /**
     * GET /api/books/{id}/exists - 書籍存在確認のテスト
     */
    @Test
    fun `checkBookExists should return true when book exists`() {
        // Given
        whenever(bookService.existsById(1L)).thenReturn(true)

        // When & Then
        mockMvc
            .perform(get("/api/books/1/exists"))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.exists").value(true))

        verify(bookService).existsById(1L)
    }

    @Test
    fun `checkBookExists should return false when book does not exist`() {
        // Given
        whenever(bookService.existsById(999L)).thenReturn(false)

        // When & Then
        mockMvc
            .perform(get("/api/books/999/exists"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.exists").value(false))

        verify(bookService).existsById(999L)
    }
    ...
    /**
     * GET /api/books/published - 出版済み書籍一覧取得のテスト
     */
    @Test
    fun `getPublishedBooks should return published books`() {
        // Given
        val publishedBooks =
            listOf(
                createSampleBookResponse(publicationStatus = PublicationStatus.PUBLISHED),
            )
        val pagedResponse =
            PagedResponse(
                content = publishedBooks,
                pageNumber = 0,
                pageSize = 20,
                totalElements = 1L,
                totalPages = 1,
        ...
        whenever(bookService.findPublishedBooks(0, 20)).thenReturn(pagedResponse)
        ...
        verify(bookService).findPublishedBooks(0, 20)
    }

    /**
     * GET /api/books/unpublished - 未出版書籍一覧取得のテスト
     */
    @Test
    fun `getUnpublishedBooks should return unpublished books`() {
        // Given
        val unpublishedBooks =
            listOf(
                createSampleBookResponse(publicationStatus = PublicationStatus.UNPUBLISHED),
            )
        val pagedResponse =
            PagedResponse(
                content = unpublishedBooks,
                pageNumber = 0,
                pageSize = 20,
                totalElements = 1L,
                totalPages = 1,
        ...
        whenever(bookService.findUnpublishedBooks(0, 20)).thenReturn(pagedResponse)
        ...
        verify(bookService).findUnpublishedBooks(0, 20)
    }

    /**
     * カスタムページネーションパラメーターのテスト
     */
    @Test
    fun `should use custom pagination parameters for published books`() {
        // Given
        val pagedResponse =
            PagedResponse(
                content = emptyList<BookResponse>(),
                pageNumber = 3,
                pageSize = 10,
                totalElements = 100L,
                totalPages = 10,
        ...
        whenever(bookService.findPublishedBooks(3, 10)).thenReturn(pagedResponse)
        ...
        verify(bookService).findPublishedBooks(3, 10)
    }
}
